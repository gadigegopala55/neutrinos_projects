...
export class idsutil {
  ...
  async getIDSClientInstance(parentSpanInst, clientInstance = null, ...others) {
    ...
    try {
      ...
      bh = await this.sd_VDEmuK3OQfeNeNbH(bh, parentSpanInst);
  ...
  async sd_VDEmuK3OQfeNeNbH(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_VDEmuK3OQfeNeNbH',
      parentSpanInst
    );
    try {
      ...
      //appendnew_next_sd_VDEmuK3OQfeNeNbH
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  async sd_NDvR3YN7qjsaaO0E(bh, parentSpanInst) {
    ...
    try {
      ...
      if (
        ...
        bh = await this.sd_LneiNQYcJEurs5Tw(bh, parentSpanInst);
        otherwiseFlag = false;
      }
      this.tracerService.sendData(spanInst, bh);
  ...
  async sd_J9L3oEbqPtl8tfbi(bh, parentSpanInst) {
    ...
    try {
      ...
      this.tracerService.sendData(spanInst, bh);
      bh = await this.sd_98T9v9TJqzjc3NmO(bh, parentSpanInst);
  ...
  async sd_98T9v9TJqzjc3NmO(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_98T9v9TJqzjc3NmO',
      parentSpanInst
    );
    try {
      ...
      this.tracerService.sendData(spanInst, bh);
      //appendnew_next_sd_98T9v9TJqzjc3NmO
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  async sd_LneiNQYcJEurs5Tw(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_LneiNQYcJEurs5Tw',
      parentSpanInst
    );
    try {
      ...
      this.tracerService.sendData(spanInst, bh);
      //appendnew_next_sd_LneiNQYcJEurs5Tw
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  async errorHandler(bh, e, src, parentSpanInst?, functionName?) {
    ...
    if (
      false ||
      (await this.sd_eVNVUC4qZt6vwk2n(bh, parentSpanInst))
  ...
  async sd_eVNVUC4qZt6vwk2n(bh, parentSpanInst) {
    const nodes = ['handleTokenExpiry'];
    if (nodes.includes(bh.errorSource)) {
      bh = await this.sd_EgUC5B8p6Vp4y70s(bh, parentSpanInst);
      //appendnew_next_sd_eVNVUC4qZt6vwk2n
      return true;
    }
    return false;
  }
